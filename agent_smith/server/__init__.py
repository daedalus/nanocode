...
class AgentServer:
    ...
    async def _session_prompt(self, request: Request) -> Response:
        ...
        try:
            if self.agent:
                response = await self.agent.process_input(messages[-1].get("content", "") if messages else "")
    ...
    async def handle_request(self, method: str, path: str, headers: dict, body: Any = None) -> Response:
